import { getEmployeeBasedOnUserId, getEmployeeMultiCompany, getResPartner } from "../utils/getOdooUserData.js";
export const employeeMultiCompanyCtrl = {
    ...
    getResPartner: async (req, res) => {
        try {
            const data = await getResPartner(req.odoo, req.user);
            res.status(200).json({ data })
        } catch (error) {
            res.status(500).json({ msg: error.message })
        }
